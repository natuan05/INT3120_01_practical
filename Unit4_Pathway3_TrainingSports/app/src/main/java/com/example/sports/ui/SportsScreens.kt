    ...
    Scaffold(
        ...
        if (contentType == SportsContentType.ListAndDetail) {
            SportsListAndDetails(
                ...
                onSportPressed = { viewModel.updateCurrentSport(it) },
                contentPadding = innerPadding, // <-- SỬA LẠI
            ...
            if (uiState.isShowingListPage) {
                SportsList(
                    ...
                    modifier = Modifier.padding(horizontal = dimensionResource(R.dimen.padding_medium)), // <-- SỬA LẠI
...
fun SportsListAndDetails(
    ...
    onSportPressed: (Sport) -> Unit,
    contentPadding: PaddingValues,
    modifier: Modifier = Modifier
) {
    val activity = LocalContext.current as Activity
    Row(modifier = modifier) {
        // Ngăn danh sách (bên trái)
        SportsList(
            sports = sportsUiState.sportsList,
            onClick = onSportPressed,
            modifier = Modifier
                .weight(1f)
                .padding(horizontal = dimensionResource(R.dimen.padding_medium)), // <-- SỬA LẠI
            contentPadding = contentPadding
        )
        ...
        SportsDetail(
            ...
            contentPadding = contentPadding,// Nút back sẽ thoát ứng dụng
