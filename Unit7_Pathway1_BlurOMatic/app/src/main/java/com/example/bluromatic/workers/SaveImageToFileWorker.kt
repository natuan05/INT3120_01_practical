        ...
        return withContext(Dispatchers.IO) {
            ...
            return@withContext try {
                ...
                if (resourceUri.isNullOrBlank()) {
                    val outputDir = File(applicationContext.filesDir, com.example.bluromatic.OUTPUT_PATH)
                    val lastFile = outputDir.listFiles()?.maxByOrNull { it.lastModified() }
                    if (lastFile != null) {
                        resourceUri = Uri.fromFile(lastFile).toString()
                    }
                }

                // Kiểm tra lại sau khi đã có phương án dự phòng
